...
export default class ContractManager extends Component {
  ...
  parseABI = () => {
    ...
    || (!utils.isEmptyObj(this.state.abiInfo) && !unichain.utils.isValidABI(this.state.abiInfo))) {
  ...
  callContractFunc = async (funcName) => {
    ...
    const contractAccount = await unichain.account.getAccountByName(this.state.contractAccount);
    ...
    const self = this;
    const payload = '0x' + unichain.utils.getContractPayload(funcName, this.state.funcParaTypes[funcName], values);
    if (this.state.funcParaConstant[funcName]) {
      const callInfo = {actionType:0, from: 'unichain.founder', to: this.state.contractAccount, assetId:0, gas:200000000, gasPrice:10000000000, value:0, data:payload, remark:''};
  ...
  getTxInfo = (funcName) => {
    ...
    if (result != null) {
      ...
      unichain.uni.getTransactionByHash(result).then(txInfo => {
  ...
  getReceiptInfo = (funcName) => {
    ...
    if (result != null) {
      ...
      unichain.uni.getTransactionReceipt(result).then(receipt => {
        if (receipt == null) {
