...
export default class RawTxConstructor extends Component {
  ...
  parseUri = async () => {
    ...
    if (typestr != 'unichain.account' && typestr != 'unichain.asset' && typestr != 'unichain.dpos' && typestr != 'unichain.contract') {
      contractAccount = await unichain.account.getAccountByName(typeStr);
    ...
    switch(actionStr) {
      case 'createAccount':
        ...
        this.state['toAccountName'] = 'unichain.account';
      ...
      case 'issueAsset':
        ...
        this.state['toAccountName'] = 'unichain.asset';
      ...
      case 'regCandidate':
        ...
        this.state['toAccountName'] = 'unichain.dpos';
  ...
  checkResult = async () => {
    ...
    if (!utils.isEmptyObj(this.state['accountName'])) {
      newFromAccount = await unichain.account.getAccountByName(this.state['accountName']);
    }
    if (!utils.isEmptyObj(this.state['toAccountName'])) {
      newToAccount = await unichain.account.getAccountByName(this.state['toAccountName']);
    }
    ...
    if (!utils.isEmptyObj(this.state.txResult) && this.state.txResult.indexOf('0x') == 0) {
      unichain.uni.getTransactionReceipt(this.state.txResult).then(receipt => {
  ...
  recordHistory = async () => {
    ...
    let toAccount = null;
    if (!utils.isEmptyObj(this.state['accountName'])) {
      fromAccount = await unichain.account.getAccountByName(this.state['accountName']);
    }
    if (!utils.isEmptyObj(this.state['toAccountName'])) {
      toAccount = await unichain.account.getAccountByName(this.state['toAccountName']);
    }
