...
export default class RawTxConstructor extends Component {
  ...
  sendTransaction = () => {
    ...
    if (txInfo.length > 130 && txInfo.charAt(0) === '{' && txInfo.charAt(txInfo.length - 1) === '}') {
      try {
        ...
        unichain.uni.signTx(txObj, this.state.privateKey).then(signInfo => {
          unichain.uni.sendSingleSigTransaction(txObj, signInfo).then(txHash => {
  ...
  getReceipt = () => {
    if (!utils.isEmptyObj(this.state.txResult) && this.state.txResult.indexOf('0x') == 0) {
      unichain.uni.getTransactionReceipt(this.state.txResult).then(resp => {
  ...
  getTxInfo = () => {
    if (this.state.txResult != null) {
      unichain.uni.getTransactionByHash(this.state.txResult).then(resp => {
        this.setState({ receipt: JSON.stringify(resp) });
      });
    } else {
  ...
  getTxCorrespondingInfo = async (testCase) => {
    ...
    const fromAccount = await unichain.account.getAccountByName(actionInfo.accountName);
    statusInfo.fromAccount = fromAccount;
    const toAccount = await unichain.account.getAccountByName(actionInfo.toAccountName);
    ...
    switch (actionType) {
      ...
      case Constant.CREATE_NEW_ACCOUNT:
        const newAccount = await unichain.account.getAccountByName(actionInfo.payloadDetailInfo.newAccountName);
      ...
      case Constant.ISSUE_ASSET:
        const owner = await unichain.account.getAccountByName(actionInfo.payloadDetailInfo.owner);
        statusInfo.owner = owner;
        break;
      case Constant.INCREASE_ASSET:
        const increaseAccount = await unichain.account.getAccountByName(actionInfo.payloadDetailInfo.accountName);
      ...
      case Constant.SET_ASSET_FOUNDER:
        // 先向unichain.asset转账，然后unichain.asset减去这笔钱，最后修改资产信息
      ...
      case Constant.VOTE_CANDIDATE:
        const candidate = await unichain.dpos.getCandidateByName(actionInfo.payloadDetailInfo.accountName);
    ...
    for (const step of procedure) {
      if (step.type == 'get') {
        ...
        try {
          ...
          testResultInfo += '\n交易原始信息：' + originalInfo;
          this.setState({ testResult: testResultInfo });
          const signInfo = await unichain.uni.signTx(step, step.privateKey);
          const txHash = await unichain.uni.sendSingleSigTransaction(step, signInfo);
          ...
          while(!gotReceipt) {
            ...
            this.setState({ testResult: testResultInfo });
            this.sleep(3000);
            const receipt = await unichain.uni.getTransactionReceipt(txHash);
            if (receipt != null) {
              gotReceipt = true;
  ...
  testSendCase = async (testCase) => {
    try {
      ...
      testResultInfo += '\n交易原始信息：' + originalInfo;
      this.setState({ testResult: testResultInfo });
      const signInfo = await unichain.uni.signTx(testCase, testCase.privateKey);
      const txHash = await unichain.uni.sendSingleSigTransaction(testCase, signInfo);
      testResultInfo += '\n本次交易发送成功，获得交易hash：' + txHash;
      this.setState({ testResult: testResultInfo });
      let gotReceipt = false;
      while(!gotReceipt) {
  ...
  getResult = (method, params) => {
    const dataToSrv = JSON.stringify({ jsonrpc: '2.0',
      ...
      params,
      id: 1 });
    return unichain.utils.postToNode({
      data: dataToSrv,
    });
  }
