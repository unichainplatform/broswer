...
export default class ContractManager extends Component {
  ...
  componentDidMount = async () => {
    ...
    unichain.uni.getSuggestionGasPrice().then(suggestionPrice => {
  ...
  checkABI = (abiInfo) => {
    ...
    || (!utils.isEmptyObj(abiInfo) && !unichain.utils.isValidABI(abiInfo))) {
  ...
  generateContractAccount = async () => {
    const nonce = await unichain.account.getNonce(this.state.selectedAccountName);
  ...
  callContractFunc = async (contractAccountName, contractName, funcName) => {
    try {      
      ...
      const contractAccount = await unichain.account.getAccountByName(contractAccountName);
      ...
      const self = this;
      const payload = '0x' + unichain.utils.getContractPayload(funcName, this.state.funcParaTypes[contractName][funcName], values);
      if (this.state.funcParaConstant[contractName][funcName]) {
        ...
        unichain.uni.call(callInfo, 'latest').then(resp => {
  ...
  getTxInfo = (contractName, funcName) => {
    ...
    if (txHash != null) {
      ...
      unichain.uni.getTransactionByHash(txHash).then(txInfo => {        
  ...
  getReceiptInfo = (contractName, funcName) => {
    ...
    if (txHash != null) {
      ...
      unichain.uni.getTransactionReceipt(txHash).then(receipt => {        
        if (receipt == null) {
  ...
  onAddContractABIOK = () => {
    if (!utils.isEmptyObj(this.state.contractABI) && !unichain.utils.isValidABI(JSON.parse(this.state.contractABI))) {
      Feedback.toast.error(T('ABI信息不符合规范，请检查后重新输入'));
      return;
    }
  ...
  sendTx = async (txInfo, fromAccount) => {
    ...
    if (keystores.length > 0) {
      ...
      for (let wallet of wallets) {
        const signInfo = await unichain.uni.signTx(txInfo, wallet.privateKey);
      ...
      if (multiSigInfos.length > 0) {   
        ...
        const fatherLevel = 0;
        return unichain.uni.sendSeniorSigTransaction(txInfo, multiSigInfos, fatherLevel);
          ...
          setTimeout(() => {
            unichain.uni.getTransactionReceipt(result.msg).then(receipt => {
              if(receipt != null) {
                ...
                if (status == 1) {
                  unichain.account.getAccountByName(self.state.firstAccountName).then(account => {
                    if (account != null) {
                      Message.show({
                        content: '账号创建成功'
                      });
        ...
        <Row className="custom-row" >
            ...
            <Col fixedSpan="15" className="custom-col-right-sidebar">
              <Row style={{width:'100%'}}>
                ...
                />
                &nbsp;&nbsp;&nbsp;
                <Button type="primary" onClick={this.copyAccount.bind(this)}>{T("复制")}</Button>
              </Row>
              <br/>
              <Row style={{width:'100%'}}>
                <Select language={T('zh-cn')}
