...
export default class AccountList extends Component {
  ...
  componentDidMount = async () => {
    try {
      unichain.dpos.getDposInfo().then(dposInfo => this.state.dposInfo = dposInfo);
      ...
      unichain.dpos.getDposIrreversibleInfo().then(irreversibleInfo => this.state.irreversibleInfo = irreversibleInfo);
  ...
  checkBlockRollback = async (blockHash, blockNum) => {
    const blockInfo = unichain.uni.getBlockByNum(blockNum);
  ...
  onImportAccountOK = async () => {
    ...
    if (this.state.idReg.test(this.state.importAccountName)) {
      ...
      try {
        unichain.account.getAccountById(accountId).then(account => {
          if (account != null) {
      ...
      try {
        unichain.account.getAccountByName(this.state.importAccountName).then(account => {
          if (account != null) {
            const accountInfos = [...self.state.accountInfos, account];
            self.setState({ accountInfos, importAccountVisible: false });
            self.saveAccountsToLS();
  ...
  showAssets = (index) => {
    ...
    for (const balance of balances) {
      if (this.state.assetInfos[balance.assetID] === undefined) {
        promiseArr.push(unichain.account.getAssetInfoById(balance.assetID));
      }
  ...
  */
  showTxs = async (index) => {
    try {
      ...
      this.state.curBlock = await unichain.uni.getCurrentBlock(false);
      this.state.curAccount = this.state.accountInfos[index];
      for (const balance of this.state.accountInfos[index].balances) {
        if (this.state.assetInfos[balance.assetID] == null) {
          const assetInfo = await unichain.account.getAssetInfoById(balance.assetID);
      ...
      if (allTxInfoSet != null) {
        ...
        if (accountTxInfoSet == null) {   // 如果在本地不存在此账号的交易信息，则从链上同步其所有交易
          await unichain.uni.getCurrentBlock(false).then(async (curBlock) => {
        ...
        for (const txInfo of accountTxInfoSet.txInfos) {
          ...
          for (const actionInfo of txInfo.actions) {
            if (this.state.assetInfos[actionInfo.assetID] === undefined) {
              const asset = await unichain.account.getAssetInfoById(actionInfo.assetID);
    ...
    if (Object.prototype.hasOwnProperty.call(txInfo, 'txStatus') && txInfo.txStatus != Constant.TxStatus.NotExecute && txInfo.txStatus != Constant.TxStatus.SendError) {   // 通过txStatus判断交易receipt是否已经获取过，条件成立则同步区块状态，否则获取交易的receipt
      if (txInfo.blockStatus == null || txInfo.blockStatus == Constant.BlockStatus.Reversible || txInfo.blockStatus == Constant.BlockStatus.Unknown) {
        if (this.state.blockRollbackCache[txInfo.blockHash] == null) {
          const blockInfo = await unichain.uni.getBlockByNum(txInfo.blockNumber);
      ...
      const receipt = await unichain.uni.getTransactionReceipt(txInfo.txHash);
      ...
      const internalTx = await unichain.uni.getInternalTxByHash(txInfo.txHash);
      if (internalTx != null) {
  ...
  */
  syncTxFromNode = async () => {
    try {
      ...
      const self = this;
      unichain.uni.getCurrentBlock(false).then(async (curBlock) => {
        ...
        for (const account of self.state.accountInfos) {
          ...
          for (; blockNum < curBlockNum; blockNum += step) {
            ...
            promiseArr.push(unichain.uni.getTxsByAccount(accountName, blockNum, lookForward));
            // 内部交易
            accountInternalTxPromiseArr.push(unichain.uni.getInternalTxsByAccount(accountName, blockNum, lookForward));
          }
          ...
          for (const txHash of accountTxs) {
            if (accountExistTxs[txHash] == null) {
              promiseArr.push(unichain.uni.getTransactionByHash(txHash));                             
            }
          }
          ...
          for (let txInfo of txInfos) {
            if (blockCache[txInfo.blockHash] == null) {
              promiseArr.push(unichain.uni.getBlockByHash(txInfo.blockHash));
            }
          }
  ...
  syncTxsOfAccount = async (account, curBlockNum) => {
    ...
    for (; blockNum >= account.number; blockNum -= step) {
      ...
      promiseArr.push(unichain.uni.getTxsByAccount(account.accountName, blockNum, lookBack + 1));
      // TODO 内部交易
      //accountTxs.push(unichain.uni.getInternalTxsByAccount(accountName, blockNum, lookBack));
    }
  ...
  getValue = async (assetId, value) => {
    ...
    if (assetInfo == null) {
      assetInfo = await unichain.account.getAssetInfoById(assetId);
    }
  ...
  renderGasFee = (value, index, record) => {
    ...
    return parseActions.map((item) => {
      ...
      const earnedGasFee = utils.getGasEarned(record.gasPrice, item.gasFee, this.state.assetInfos[record.gasAssetID]) + 'uni';
  ...
  renderGasAllot = (value, index, record) => {
    ...
    return parseActions[0].gasAllot.map((gasAllot) => {
      ...
      const earnedGasFee = utils.getGasEarned(record.gasPrice, gasAllot.gas, this.state.assetInfos[record.gasAssetID]) + 'uni';
  ...
  onSystemCreatAccountOK = () => {
    if (!this.state.newAccountReg.test(this.state.unichainAccount)) {
      Feedback.toast.error(T('账号格式错误'));
      return;
    }
    if (this.state.unichainPublicKey == '') {
    ...
    // this.props.createAccountBySystem({ accountName: this.state.unichainAccount, publicKey: this.state.unichainPublicKey, email: this.state.email });
  };
  ...
  onSelfCreateAccountOK = async () => {
    ...
    const exist = await unichain.account.isAccountExist(this.state.newAccountName);
    if (exist) {
    ...
    this.state.curAccount = await unichain.account.getAccountByName(this.state.creator);
        ...
        setTimeout(() => {
          unichain.uni.getTransactionReceipt(result.msg).then(receipt => {
            ...
            if(receipt != null) {
              ...
              if (status == 1) {
                unichain.account.getAccountByName(self.state.newAccountName).then(account => {
                  if (account != null) {
  ...
  onBindNewAuthorOK = async () => {
    ...
    if (newOwner.indexOf('.') > 0 || newOwner.length <= 8) {
      const exist = await unichain.account.isAccountExist(newOwner);
      if (!exist) {
  ...
  onWithdrawTxFeeOK = async () => {
    ...
    if (this.state.txFeeType == TxFeeType.Asset) {
      const assetInfo = await unichain.account.getAssetInfoByName(txFeeName);
      const assetId = assetInfo.assetId == null ? 0 : assetInfo.assetId;
      payload += unichain.utils.getContractPayload('withdrawAssetFee', ['uint256'], [assetId]);
    } else if (this.state.txFeeType == TxFeeType.Contract) {
      const account = await unichain.account.getAccountByName(txFeeName);
      payload += unichain.utils.getContractPayload('withdrawAccountFee', ['uint256'], [account.accountID]);
    } else if (this.state.txFeeType == TxFeeType.Coinbase) {
      const account = await unichain.account.getAccountByName(txFeeName);
      payload += unichain.utils.getContractPayload('withdrawCoinbaseFee', ['uint256'], [account.accountID]);
    }
  ...
  getTxFee = async () => {
    const txFeeName = this.state.txFeeType == TxFeeType.Asset ? this.state.assetName : this.state.curAccount.accountName;
    const txFeeInfoObj = await unichain.fee.getObjectFeeByName(txFeeName, this.state.txFeeType);
    ...
    if (txFeeInfoObj == null) {
      ...
      if (this.state.txFeeType == TxFeeType.Asset) {
        ...
        const accountInfo = await unichain.account.getAccountByName(txFeeName);
  ...
  symbolRender = (value) => {
    ...
    const self = this;
    unichain.account.getAssetInfoById(value).then(assetInfo => {
  ...
  transfer = (index) => {
    ...
    const self = this;
    unichain.uni.getSuggestionGasPrice().then(gasPrice => {
  ...
  onTransferOK = () => {
    ...
    const self = this;
    unichain.account.isAccountExist(this.state.transferToAccount).then(exist => {
      if (!exist) {
  ...
  onAddContractABIOK = () => {
    if (!utils.isEmptyObj(this.state.contractABI) && !unichain.utils.isValidABI(this.state.contractABI)) {
