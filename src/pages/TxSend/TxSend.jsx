...
export default class TxSend extends Component {
  ...
  onChangeAccount = (accountName) => {
    ...
    unichain.account.getAccountByName(accountName).then(account => this.state.curAccount = account);
    this.addSuperAccount(accountName);
  }

  onChangeSuperAccount = (superAccountName) => {
    this.state.superAccountName = superAccountName;
    unichain.account.getAccountByName(superAccountName).then(account => this.state.curSupperAccount = account);
  }
  ...
  onTxConfirmOK = () => {
    ...
    if (keystores.length == 0) {
      ...
      unichain.uni.packTx(txInfo).then(txToBeSigned => {
      ...
      Promise.all(promiseArr).then(async (wallets) => {
        ...
        if (multiSigInfos.length == 0) {  // 这条路径暂时关闭
          unichain.uni.sendSingleSigTransaction(txInfo, multiSigInfos[0].signInfo).then(txHash => {
  ...
  onSignOK = () => {
    ...
    unichain.uni.sendSeniorSigTransaction(txInfo, multiSigInfos, 0).then(txHash => {
      console.log('tx hash=>' + txHash);
      this.processTxSendResult(txInfo, txHash);
      this.onTxConfirmClose();
      if (this.props.sendResult != null) {
  ...
  onLoadData(data) {
    ...
    unichain.account.getAccountByName(accountName).then(async (account) => {
      if (account == null) {
        return;
      }
