...
export default class CandidateList extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      for (const account of accountInfos) {
        ...
        const voters = await unichain.dpos.getVotersByVoter(0, account.accountName, true);
  ...
  updateCandidateInfo = () => {
    unichain.dpos.getCandidates(0, true).then(candidates => {
      unichain.dpos.getValidCandidates(0).then(validCandidates => {
        ...
        candidates.map(candidate => {
          ...
          unichain.dpos.getEpochByHeight(candidate.number).then(epoch => {
        ...
        let index = 0;
        validCandidates.activatedCandidateSchedule.map(validCandidate => {
          unichain.dpos.getVotersByCandidate(0, validCandidate, true).then(voters => {
            this.state.votersOfAllCandidate[validCandidate] = voters;
          });
          unichain.dpos.getActivedCandidate(0, index++).then(candidateData => {
  ...
  updateInfo = () => {
    unichain.uni.getCurrentBlock(false).then(block => {
      this.state.curBlock = block;
      unichain.dpos.getEpochByHeight(block.number).then(epoch => {
  ...
  updateProducer = () => {
    ...
    unichain.account.getAccountByName(candidateName, true).then(async (account) => {
  ...
  refundReposit = async () => {
    ...
    const freezeEpoch = await unichain.dpos.getEpochByHeight(this.state.curSelectedCandidate.number);
    let passedEpochNum = 0;
    try {
      let nextEpoch = await unichain.dpos.getNextEpoch(freezeEpoch);
      while (nextEpoch <= this.state.curEpoch) {
        passedEpochNum++;
        nextEpoch = await unichain.dpos.getNextEpoch(nextEpoch);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  getReward = () => {
    ...
    const accountName = this.state.rowSelection.selectedRowKeys[0];
    const payload = '0x' + unichain.utils.getContractPayload('producersGetReward', [], []);
  ...
  getAccountStake = async (account) => {
    const stake = await unichain.dpos.getAvailableStake(0, account.accountName);
