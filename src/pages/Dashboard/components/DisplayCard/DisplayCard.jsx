...
class BlockTxLayout extends Component {
  ...
  updateBlockChainInfo = () => {
    try {
      const self = this;
      unichain.dpos.getDposIrreversibleInfo().then(irreversibleInfo => {
        this.setState({ irreversible: irreversibleInfo });
      });
      unichain.dpos.getValidCandidates(0).then(latestEpchoInfo => {
        this.setState({ latestEpchoInfo, activeProducers: latestEpchoInfo.activatedCandidateSchedule });
      });
      unichain.dpos.getCandidates(0, false).then(candidates => {
        this.setState({ curProducerList: candidates });
      });
      unichain.dpos.getDposInfo().then(dposInfo => {
        unichain.uni.getCurrentBlock(false).then(curBlockInfo => {
