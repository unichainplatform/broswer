...
export default class KeyList extends Component {
  ...
  signTxInfo = () => {
    try {
      ...
      unichain.uni.signTx(txInfoObj, this.state.msgContent).then(signature => {
  ...
  verifySignInfo = () => {
    try {
      ...
      unichain.uni.recoverSignedTx(txInfoObj, this.state.signResult).then(address => {
  ...
  processAction = (filterFunc, toastStr, succssFunc) => {
    const keystoreInfo = utils.getDataFromFile(KeyStoreFile);
    const unichainKSInfo = keystoreInfo.keyList.filter(filterFunc);
  ...
  onChangeChainId = (chainId) => {
    unichain.uni.setChainId(chainId);
    this.loadKeyInfo();
  }
