...
export default class AssetIssueTable extends Component {
  ...
  onSubmit = async () => {
    ...
    try {
      const assetInfo = await unichain.account.getAssetInfoByName(value.assetName);
      if (assetInfo != null) {
    ...
    const accountInfo = await unichain.account.getAccountByName(value.assetName);
    if (accountInfo != null) {
    ...
    for (const fatherAssetName of fatherAssetNames) {
      let assetInfo = null;
      try {
        assetInfo = await unichain.account.getAssetInfoByName(fatherAssetName);        
      } catch (error) {
    ...
    let bExist = await unichain.account.isAccountExist(value.owner);
    if (!bExist) {
      Feedback.toast.error(T('管理者账户不存在'));
      return;
    }
    bExist = await unichain.account.isAccountExist(value.founder);
    if (!bExist) {
