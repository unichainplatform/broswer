...
export default class TransactionList extends Component {
  ...
  componentDidMount() {
    unichain.account.getAssetInfoById(0).then(assetInfo => {
  ...
  getTxInfoByBlock(blockHeight) {
    unichain.uni.getBlockByNum(blockHeight, true).then(async(curBlockInfo) => {
  ...
  getTxInfoByTxHash(txHashArr) {
    ...
    txHashArr.map(txHash => {
      if (this.state.txHashSet[txHash] == null) {
        txPromiseArr.push(unichain.uni.getTransactionByHash(txHash));
      }
    });
  ...
  processTxs = async (txInfos) => {
    ...
    for (const transaction of txInfos) {
      const internalTx = await unichain.uni.getInternalTxByHash(transaction.txHash);
      if (internalTx != null) {
        transaction.innerActions = internalTx.actions[0].internalActions;
      } else {
        transaction.innerActions = [];
      }
      receiptPromiseArr.push(unichain.uni.getTransactionReceipt(transaction.txHash));
      txSet[transaction.txHash] = transaction;
    }
    const dposInfo = await unichain.dpos.getDposInfo();
        ...
        for (const receipt of receipts) {
          ...
          for (let actionInfo of transaction.actions) {
            if (_this.state.assetInfos[actionInfo.assetID] == null) {
              const assetInfo = await unichain.account.getAssetInfoById(actionInfo.assetID);
